    ...
    <div className="overflow-x-auto">
      <table className="min-w-full text-sm">
        <thead className="bg-slate-50">
          <tr>
            <th className="px-4 py-2 text-left text-xs font-semibold uppercase text-slate-500">Sr.</th>
            ...
            <th className="px-4 py-2 text-right text-xs font-semibold uppercase text-slate-500">Qty</th>
        ...
        <tbody className="divide-y divide-slate-100">
          {lines.map((line, index) => (
            <tr key={line.id ?? index}>
              ...
              {showAccount && (
                <td className="px-4 py-2 text-slate-600">{accountLabel(line.accountId)}</td>
              )}
              ...
              <td className="px-4 py-2 text-right text-slate-700">{formatNumber(line.quantity)}</td>
        ...
        <tfoot className="bg-slate-50">
          <tr>
            <td colSpan={showAccount ? 6 : 5} className="px-4 py-3 text-right text-sm font-medium text-slate-700">
              Total
            </td>
