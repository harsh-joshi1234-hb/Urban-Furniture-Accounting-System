...
export default function NewInvoicePage() {
  ...
  const selectedCustomer = (customers.data ?? []).find(
    (contact) => contact.id === form.customerId,
  );
  ...
  return (
    <div className="mx-auto max-w-6xl">
      ...
      <form onSubmit={onSubmit} noValidate className="space-y-4">
        ...
        {/*
          Portal visibility follows the customer, not the invoice: whoever has access to
          this customer will see the invoice once it is confirmed. Surfacing it here means
          a new invoice is never silently invisible to the people meant to pay it.
        */}
        {selectedCustomer ? (
          <PortalAccessCard
            className=""
            ...
            title="Who will see this invoice"
            subtitle={`Portal accounts with access to ${selectedCustomer.name}. Grant access here and it applies to this invoice too.`}
          />
        ) : (
          <Card title="Who will see this invoice" bodyClassName="p-4">
            <p className="text-sm text-stone-500">
              Select a customer to see and assign the portal accounts that will be able to
              view this invoice.
            </p>
          </Card>
        )}
